...
class HUWebshop(object):
    ...
    recommendationtypes = {'popular': "Anderen kochten ook", 'similar': "Soortgelijke producten",
                           'combination': 'Vaak samengekocht met', 'behaviour': 'Passend bij uw gedrag',
    ...
    def renderpackettemplate(self, template="homepage.html", packet={}):
        ...
        if 'r_products' not in packet and 'r_type' not in packet and 'r_string' not in packet:
            packet['r_products'] = self.recommendations(4, list(self.recommendationtypes.keys())[3])
            packet['r_type'] = list(self.recommendationtypes.keys())[3]
            packet['r_string'] = list(self.recommendationtypes.values())[3]

        return render_template(template, packet=packet)

    """ ..:: Recommendation Functions ::.. """

    def recommendations(self, count, recommendationtype, product_id=None, cat=None):
        ...
        if recommendationtype == "popular":
            resp = requests.get(self.recseraddress + "/" + cat + "/" + str(count) + "/" + str(recommendationtype))
        elif recommendationtype == "similar":
            resp = requests.get(
                self.recseraddress + "/" + product_id + "/" + str(count) + "/" + str(recommendationtype))
        elif recommendationtype == "combination":
            if product_id is None:
                resp = requests.get(
                    self.recseraddress + "/" + '1932' + "/" + str(count) + "/" + str(recommendationtype))
            else:
                resp = requests.get(self.recseraddress + "/" + product_id + "/" + str(count) + "/" + str(recommendationtype))
        elif recommendationtype == "behaviour":
            resp = requests.get(
                self.recseraddress + "/" + session['profile_id'] + "/" + str(count) + "/" + str(recommendationtype))
        elif recommendationtype == "personal":
            resp = requests.get(self.recseraddress + "/" + session['profile_id'] + "/" + str(count) + "/" + str(recommendationtype))
    ...
    def productpage(self, cat1=None, cat2=None, cat3=None, cat4=None, page=1):
        ...
        return self.renderpackettemplate('products.html', {'products': prodlist,
                                                           ...
                                                           'r_products': self.recommendations(4, list(
                                                               self.recommendationtypes.keys())[0], cat=cat1),
    ...
    def productdetail(self, productid):
        ...
        return self.renderpackettemplate('productdetail.html', {'product': product,
                                                                ...
                                                                'r_products': self.recommendations(4, list(
                                                                    self.recommendationtypes.keys())[1],
                                                                                                   str(productid)),
